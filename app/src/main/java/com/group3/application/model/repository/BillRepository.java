...
public class BillRepository {
    ...
    public LiveData<Resource<BillDetailResponse>> getBillDetails(String billId) {
        MutableLiveData<Resource<BillDetailResponse>> result = new MutableLiveData<>();
        result.setValue(Resource.loading(null));
        apiService.getBillDetails(billId).enqueue(new Callback<BillDetailResponse>() {
            @Override
            public void onResponse(Call<BillDetailResponse> call, Response<BillDetailResponse> response) {
                if (response.isSuccessful() && response.body() != null) {
                    result.setValue(Resource.success(response.body()));
                } else {
                    result.setValue(Resource.error("Bill not found", null));
                }
            }
            @Override
    ...
    public LiveData<Resource<PaymentConfirmationResponse>> confirmPayment(String billId, String method) {
        MutableLiveData<Resource<PaymentConfirmationResponse>> result = new MutableLiveData<>();
        result.setValue(Resource.loading(null));

        PaymentConfirmationRequest request = new PaymentConfirmationRequest(method);

        apiService.confirmPayment(billId, request).enqueue(new Callback<PaymentConfirmationResponse>() {
            @Override
            public void onResponse(Call<PaymentConfirmationResponse> call, Response<PaymentConfirmationResponse> response) {
                if (response.isSuccessful() && response.body() != null) {
                    result.setValue(Resource.success(response.body()));
                } else {
                    result.setValue(Resource.error("Failed to confirm payment", null));
                }
            }
    ...
    public LiveData<Resource<List<BillSummaryDTO>>> getBillList(String date) {
        MutableLiveData<Resource<List<BillSummaryDTO>>> result = new MutableLiveData<>();
        result.setValue(Resource.loading(null));
        apiService.getBillList(date).enqueue(new Callback<List<BillSummaryDTO>>() {
            @Override
            public void onResponse(Call<List<BillSummaryDTO>> call, Response<List<BillSummaryDTO>> response) {
                if (response.isSuccessful() && response.body() != null) {
                    result.setValue(Resource.success(response.body()));
                } else {
                    result.setValue(Resource.error("Failed to get bill list", null));
                }
            }
            @Override
            public void onFailure(Call<List<BillSummaryDTO>> call, Throwable t) {
                result.setValue(Resource.error(t.getMessage(), null));
            }
        });
        return result;
